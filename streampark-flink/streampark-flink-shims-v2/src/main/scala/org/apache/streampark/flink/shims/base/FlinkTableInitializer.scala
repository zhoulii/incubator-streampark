...
object FlinkTableInitializer {
  ...
  def initialize(args: Array[String], config: (TableConfig, ParameterTool) => Unit): (ParameterTool, TableEnvironment) = {
    if (flinkInitializer == null) {
      this.synchronized {
        if (flinkInitializer == null) {
          flinkInitializer = new FlinkTableInitializer(args, ApiType.java)
    ...
    tableMode match {
      ...
      case TableMode.streaming =>
        ...
        if (javaStreamEnvConfFunc != null) {
          javaStreamEnvConfFunc.configuration(streamEnvironment, parameter)
        }
